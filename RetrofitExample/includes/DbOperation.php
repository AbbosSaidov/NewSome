...
class DbOperation
{
    ...
    //Method for id
    function registerID($UniqueId)
    {

        if (!$this->isIDExist($UniqueId)) {
            $stmt = $this->con->prepare("INSERT INTO players (UniqueID) VALUES (?)");
    ...
    function isIDExist($email)
    {
        $stmt = $this->con->prepare("SELECT UniqueID FROM players WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();
        return $stmt->num_rows > 0;
